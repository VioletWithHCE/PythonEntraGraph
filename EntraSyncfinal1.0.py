...
Setup:
python -m venv entratest1
source entratest1/bin/activate
pip install msgraph-sdk mysql-connector-python

MySQL setup:
sudo mysql -u root

CREATE USER 'sqltest1'@'localhost' IDENTIFIED WITH caching_sha2_password BY 'password';
GRANT CREATE, SELECT, INSERT, UPDATE, DROP ON entra_data.* TO 'sqltest1'@'localhost';
GRANT CREATE, DROP ON *.* TO 'sqltest1'@'localhost';
FLUSH PRIVILEGES;
...
CREATE TABLE IF NOT EXISTS entra_users (
    ...
    licenses TEXT,
    password_policies VARCHAR(255),
    mobile_phone VARCHAR(50),
    business_phones VARCHAR(255),
...
CREATE TABLE IF NOT EXISTS entra_devices (
    ...
    primary_user_upn VARCHAR(255),
    primary_user_email VARCHAR(255),
    primary_user_display_name VARCHAR(255),
    last_synced DATETIME
)
""")
...
CREATE TABLE IF NOT EXISTS entra_groups (
    ...
    membership_rule_processing_state VARCHAR(50),
    proxy_addresses TEXT,
    on_premises_sync_enabled BOOLEAN,
    on_premises_domain_name VARCHAR(255),
    has_teams BOOLEAN,
    is_dynamic BOOLEAN,
    last_synced DATETIME
)
...
# ── Upsert Helpers ────────────────────────────────────────────────────────────
def upsert_user(cursor, user, license_names):
    sql = """
INSERT INTO entra_users (
    ...
    fax_number, last_password_change, last_synced
) VALUES (
    %s, %s, %s, %s, %s, %s,
    %s, %s, %s, %s, %s, %s,
    %s, %s, %s, %s,
...
"""
    ...
    cursor.execute(sql, (
        ...
        user.fax_number, user.last_password_change_date_time, now,
    ))
...
async def sync_users(client: GraphServiceClient, conn, limit: int = 999):
    ...
    while result:
        for user in result.value:
            ...
            upsert_user(cursor, user, license_names)
            count += 1
            print(f"  Synced user {count}: {user.display_name}")
    ...
    conn.commit()
    cursor.close()
    print(f"\nDone. {count} users synced.")

async def sync_devices(client: GraphServiceClient, conn, limit: int = 999):
    ...
    while result:
        for device in result.value:
            try:
                ...
                count += 1
                print(f"  Synced device {count}: {device.device_name}")
            except Exception as e:
                print(f"  Skipped device {device.device_name}: {e}")
        if result.odata_next_link:
    ...
    conn.commit()
    cursor.close()
    print(f"\nDone. {count} devices synced.")

async def sync_groups(client: GraphServiceClient, conn, limit: int = 999):
    ...
    while result:
        for group in result.value:
            try:
                ...
                count += 1
                print(f"  Synced group {count}: {group.display_name}")
            except Exception as e:
                print(f"  Skipped group {group.display_name}: {e}")
        if result.odata_next_link:
...
async def sync_shared_mailboxes(client: GraphServiceClient, conn, limit: int = 999):
    config = RequestConfiguration()
    config.query_parameters = UsersRequestBuilder.UsersRequestBuilderGetQueryParameters(
    ...
    # Pre-fetch all groups once to map email -> group_id
    ...
    for user in result.value:
        ...
        upsert_mailbox(cursor, user.id, user.display_name, user.mail,
                       forward_to, owners, members)
        count += 1
        print(f"  Synced {count}: {user.display_name} <{user.mail}>")

    conn.commit()
    cursor.close()
    print(f"\nDone. {count} shared mailboxes synced.")

...
Changes in 1.4:
- Added change history logger → /var/log/entrachanges.log
- Change detection on all four tables (ADDED, CHANGED, DELETED)
"""
...
def fetch_existing(conn, table, id_col="id"):
    """Return dict of {id: {col: val}} for all rows in table."""
...
def normalize(val):
    """Normalize values for comparison — booleans, None, and strings."""
    if val is None:
        return ""
...
# ── Sweep Helper ──────────────────────────────────────────────────────────────
def sweep_deleted(conn, table, entra_ids, label_col=None):
    cursor = conn.cursor(dictionary=True)
    cursor.execute(f"SELECT id, {label_col} FROM {table}" if label_col else f"SELECT id FROM {table}")
    rows = cursor.fetchall()
    cursor.close()

    db_ids = {str(row["id"]) for row in rows}
    label_map = {str(row["id"]): row.get(label_col, row["id"]) for row in rows} if label_col else {}
    ...
    if removed:
        ...
        for rid in removed:
            ...
            change_logger.info(f"DELETED | {table} | {label}")
            logging.error(f"Sweep: deleted record from {table}: {rid}")
...
async def sync_groups(client, conn, limit=999):
    ...
    tracked = [
        ...
        "mail_enabled", "security_enabled", "visibility",
        "membership_rule", "has_teams", "is_dynamic",
        "on_premises_sync_enabled", "on_premises_domain_name",
    ]
